...
from models.database import init_db, insert_sensor, get_all_sensors, update_sensor, insert_reading, get_readings, get_sensor_by_identifier, get_notifications, insert_notification
import json
import datetime
import csv
...
@app.route('/sensors/<identifier>', methods=['PUT'])
def update_sensor_config(identifier):
    data = request.get_json()
    try:
        sensor = {
            'identifier': data['identifier'],
        ...
        success = update_sensor(identifier, sensor)
        if success:
